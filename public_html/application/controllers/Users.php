...
class Users extends CI_Controller {
	...
	public function single_sign_on( $account_id, $user_id )
	{
		...
		$customer_key = $account['elucidat_public_key'];
		$secret = $this->config->item('elucidat_api_secret');
		$signon_end_point = $this->config->item('elucidat_app_url').'single_sign_on/login';
		// 
		$this->load->library('elucidat');
		...
		// build the signature
		$oauth_params['oauth_signature'] = $this->elucidat->build_signature($secret, $oauth_params, 'GET', $signon_end_point);
		// now encode for the request
		$request_token = $this->elucidat->build_base_string($oauth_params,'&');
